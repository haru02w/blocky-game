...
std::unordered_map<int, std::string> Window::keyNames = {
    /* Printable keys */
...
Window::Window(const std::string &title, int width, int height)
{
    ...
    if (glfwInit() == GLFW_FALSE) {
        const std::string &error = "Failed to initialize GLFW";
...
void Window::setupInputEvents()
{
    ...
    glfwSetCursorPosCallback(
        mWindow, [](GLFWwindow *window, double xpos, double ypos) {
            Window::Size scrSize;
            glfwGetWindowSize(window, &scrSize.width, &scrSize.height);
            globals.eventManager.dispatch("MOUSE_MOVEMENT",
                MouseEvent(xpos, ypos, scrSize.width, scrSize.height));
        });
}
